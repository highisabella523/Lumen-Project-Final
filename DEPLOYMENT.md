...
7. Deploy with `PSIPHON_ENABLED=false` first. This is the safe default and
...
| Optional Psiphon VLESS WebSocket | `/ws/p-core/cq/{UUID}` | Managed loopback-only Psiphon Core SOCKS tunnel |
...
backend selector. The Psiphon route never invokes the normal proxy resolver
...
## Optional Psiphon Core transport

The project does **not** ship an official Psiphon Tunnel Core binary, server
entries, or Core configuration. Set `PSIPHON_ENABLED=true` only after placing
an operator-authorized official ConsoleClient binary and configuration where
the unprivileged `lumen` user can read/execute them. Prefer a read-only secret
or volume mount for the source configuration. The application creates a
derived mode-`0600` runtime config and deletes it when the Core process stops.

| Variable | Requirement | Safe format/example | Secret? |
|---|---|---|---|
| `PSIPHON_ENABLED` | Optional; safe default is disabled | `false` or `true` | No |
| `PSIPHON_CONSOLE_CLIENT_PATH` | Required only when Psiphon is enabled; absolute executable path | `/run/psiphon/ConsoleClient` | No |
| `PSIPHON_CONFIG_PATH` | Required only when Psiphon is enabled; absolute authorized Core JSON file | `/run/psiphon/client.json` | **Yes** |
| `PSIPHON_DATA_DIR` | Optional persistent non-secret Core/manager state location | `/data/psiphon` | No |
| `PSIPHON_RUNTIME_DIR` | Optional ephemeral directory for the derived mode-`0600` Core config | `/tmp/lumen-psiphon-runtime` | No |
| `PSIPHON_SESSION_MAX_AGE_SECONDS` | Optional, 60–1800; hard-capped at 30 minutes | `1800` | No |
| `PSIPHON_HEALTHCHECK_INTERVAL_SECONDS` | Optional, 10–900 | `60` | No |
| `PSIPHON_CONNECT_TIMEOUT_SECONDS` | Optional, 5–120 | `45` | No |
| `PSIPHON_ROTATION_GRACE_SECONDS` | Optional, 1–60 | `15` | No |
| `PSIPHON_MAX_RECONNECT_ATTEMPTS` | Optional, 1–8 | `3` | No |
| `PSIPHON_RECONNECT_BACKOFF_SECONDS` | Optional, 1–60 | `5` | No |
| `PSIPHON_EGRESS_REGION` | Optional ISO 3166-1 alpha-2 preference; omit for Core server selection | `DE` | No |
| `PSIPHON_USE_MANAGED_UPSTREAM_PROXY` | Optional; uses only validated managed HTTP/SOCKS4a/SOCKS5 upstream candidates | `false` | No |

With `PSIPHON_ENABLED=false`, no Core process, Psiphon health worker, or
Psiphon subscription profile is created. With it enabled but a prerequisite
missing, the app remains healthy for `/ws/{UUID}` and reports a safe Psiphon
unavailable reason via the authenticated status endpoint. It does not publish
the Psiphon profile or fall back to the normal proxy backend.

When Core reaches `HEALTHY`, the normal subscription encoder appends an
ordinary VLESS+WebSocket profile using the same UUID and exactly:

```text
path=/ws/p-core/cq/{UUID}
```

The Core SOCKS listener is accepted only as `127.0.0.1`; it is never exposed
by Railway. UDP is not advertised and remains `NOT_SUPPORTED`.
...
curl -fsS -b "session=..." https://YOUR_PUBLIC_DOMAIN/api/psiphon/status
```

Verify the old `/ws/{UUID}` client profile first. Enable Psiphon only in
