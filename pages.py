...
function renderPsiphonStatus(data){
  const stateEl=document.getElementById('psiphon-status-state'),grid=document.getElementById('psiphon-status-grid'),note=document.getElementById('psiphon-status-note');
  if(!stateEl||!grid)return;
  const state=String(data?.state||'UNAVAILABLE'),healthy=Boolean(data?.available),failed=state==='FAILED';
  stateEl.className='psiphon-status-state '+(healthy?'healthy':(failed?'failed':''));
  stateEl.textContent=state;
  const cell=(label,value)=>`<div class="psiphon-status-cell"><span>${esc(label)}</span><strong>${esc(value||'—')}</strong></div>`;
  grid.innerHTML=[
    cell('Psiphon WS',data?.psiphon_ws_status),
    cell('Core availability',data?.core_available?'Configured':'Unavailable'),
    cell('Selected location',data?.selected_location),
    cell('Tunnel setup',data?.tunnel_setup_ms==null?'—':proxyMs(data.tunnel_setup_ms)),
    cell('Session age',psiphonDuration(data?.session_age_seconds)),
    cell('Remaining lifetime',psiphonDuration(data?.remaining_lifetime_seconds)),
    cell('Next rotation',data?.next_rotation_at),
    cell('Rotation count',String(data?.rotation_count??0)),
    cell('Last rotation',data?.last_rotation_at),
    cell('Last proxy re-test',data?.last_proxy_retest_at),
    cell('Last health check',data?.last_successful_health_check_at),
    cell('TCP tunnel health',data?.tcp_tunnel_health),
    cell('UDP tunnel health',data?.udp_tunnel_health),
    cell('Active Psiphon WS',String(data?.active_psiphon_ws_sessions??0)),
    cell('Exit IP',data?.exit_ip),
    cell('Exit region',data?.exit_region||data?.exit_country),
  ].join('');
  if(note)note.textContent=!data?.enabled
    ?'Disabled by PSIPHON_ENABLED. No Core process or Psiphon worker is running.'
    :(data?.last_error
      ?`Unavailable or degraded: ${String(data.last_error)}. Existing VLESS/WebSocket traffic is unaffected.`
      :'The loopback-only Core proxy is never exposed publicly. The additive standard VLESS/WS profile appears only while the Core tunnel is healthy.');
}
