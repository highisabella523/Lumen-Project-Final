...
ingress remains ``/ws/{uuid}``; the optional Psiphon route is another
...
check(
    'isolated Psiphon WS route is registered before generic UUID route',
    main.find('add_api_websocket_route("/ws/p-core/cq/{uuid}"')
    < main.find('add_api_websocket_route("/ws/{uuid}"'),
)
relay = (ROOT / 'relay_vless.py').read_text(encoding='utf-8').lower()
psiphon_relay = (ROOT / 'relay_psiphon.py').read_text(encoding='utf-8').lower()
check('protected WS relay has no Psiphon dependency', 'psiphon' not in relay)
check('Psiphon relay does not call normal outbound backend', 'open_outbound(' not in psiphon_relay)
check('Psiphon relay does not resolve existing proxy selection', '_resolve_exact_selection(' not in psiphon_relay)
check('WS VLESS serialization delegated to registry', 'TRANSPORTS.vless_parameters(' in main)
check('legacy links receive empty transport settings', '_stored_settings = _link.get("transport_settings")' in main)
